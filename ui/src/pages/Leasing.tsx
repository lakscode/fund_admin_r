...
import leasingFallback from '../data/leasing.json';
import { getLeasing } from '../services/api';
...
function Leasing() {
  const [data, setData] = useState<LeasingData>(leasingFallback);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState(leasingFallback.tabs[0]);
  ...
  useEffect(() => {
    getLeasing()
      .then((apiData) => {
        setData(apiData);
        setActiveTab(apiData.tabs?.[0] || leasingFallback.tabs[0]);
      })
      .catch(() => {
        setData(leasingFallback);
      })
      .finally(() => setLoading(false));
  }, []);

  const { page, tabs, kpiCards, chartData, actionQueue } = data;
    ...
    return (
      <div className="dashboard-layout">
        ...
        <div className="dashboard-main">
          <Topbar title="Leasing" onMenuToggle={toggleSidebar} />
          <div className="leasing-content">
            <div className="loading-overlay">
              ...
              <p className="loading-text">Loading leasing data...</p>
            </div>
          </div>
        </div>
      </div>
